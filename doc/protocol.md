...
控制衣架拾取衣物。衣物的命令有提取和归位。`fetch` 控制将衣物取出来，`homing`控制衣物归位
...
```json
{
  ...
  "from": 0,
  "packet_type": "Command",
  "command": "Actuator-Control",
  // 指明是执行器控制命令
  "actuator": "Shelf",
  // 指明控制元件，这里是衣架
  "action": "fetch",
  // 可替换为 "homing" 来归位衣服
  "remark": "Command to fetch the cloth to the default position.",
  // 备用信息栏目
  "cloth": {
    "id": "12345",
    // 唯一标识符
