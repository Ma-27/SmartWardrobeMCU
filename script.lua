...
-----------------------------------------------------------------------------------------------------------
-- 解析设备上传数据
function device_data_analyze()
    -- 假设 dev:receive() 能够接收到设备上传的JSON字符串
    local json_str = dev:receive()

    -- 手动解析特定格式的JSON字符串
    local temperature = json_str:match('"temperature":([%d%.]+)')
    ...
    -- 准备解析后的数据点集合
    local points = {}
    local timestamp = os.time() -- 使用服务器时间作为时间戳

    -- 添加温度数据点
    table.insert(points, {
        i = "temperature",
        a = timestamp,
        v = tonumber(temperature)
    })
    ...
    -- 这里我们假设有一个 to_json 函数可以将表转换为JSON字符串
    -- 因为我们没有 json 库，所以这需要你根据实际情况实现
    local points_json = to_json(points)

    -- 响应设备
    dev:response()
    dev:send("server received data")

    return #points_json, points_json
end
