...
void setup() {
    ...
    // 测试AT+RST 能否复位
    // Serial.println("Testing AT+RST command...");
    // while (!at_exec("AT+RST", "OK", 5000));           // 增加超时时间

    // 设置工作模式。1：station模式；2：ap模式；3：ap+station模式
    Serial.println("Setting AT+CWMODE=3 command...");
    ...
    // 查询本机IP
    Serial.println("Querying IP address...");
    ...
    Serial.println("Setting single connection with AT+CIPMUX=0...");
    while (!at_exec("AT+CIPMUX=0", "OK", 2000));

    // 设置透传模式。0非透传模式；1透传模式
    Serial.println("Setting AT+CIPMODE=1 for transparent mode...");
